...
hace falta alta manual. Un agente marcado como **gestor** (hoy,
`agalvez@inmobiliariapalanca.com`) tiene además una pestaña "Equipo" con
dos vistas: el plan de cada agente por separado, o todas las acciones de
todo el equipo juntas en una sola lista. Puede ver y editar el plan de
cualquiera. Login con Google ya probado y funcionando en producción:
https://plan-marketing-agente.vercel.app
...
│   └── admin/
│       ├── agentes.js              # GET (solo gestor): todos los agentes + resumen de su plan
│       └── acciones.js             # GET (solo gestor): todas las acciones de todo el equipo, en una lista
├── src/
...
`agentes.es_admin` (columna booleana; en la interfaz se llama "Gestor")
marca quién ve la pestaña "Equipo" y puede ver/editar el plan de
cualquiera. Hoy solo `agalvez@inmobiliariapalanca.com` lo es. Para dar de
alta a otro gestor (necesita haber iniciado sesión al menos una vez, para
que su perfil ya exista):
